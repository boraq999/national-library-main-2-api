...
const AcademicSearchPage = () => {
  ...
  const handleSearch = async (e) => {
    ...
    setNotFound(false);
    try {
      // إرسال طلب إلى API الخارجي فقط بدون أي فلترة أو معالجة
      const apiUrl = `https://alalem.c-library.org/api/thesis-titles-simple-search?q=${encodeURIComponent(search)}`;
      ...
      <Container >
        ...
        <Box component="form" onSubmit={handleSearch} sx={{ display: 'flex', gap: 2, mb: 4, justifyContent: 'center' }}>
          ...
          <Button
            ...
            disabled={loading || search.trim().length < 10}
